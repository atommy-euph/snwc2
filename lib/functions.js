...
export const getRandomRange = (min, max, answer, answers) => {
  ...
  if (a > b) [a, b] = [b, a];
  let possibleCandidates = getCandidates(answer, answers).filter((value) => {
  ...
  return [a, b];
  // const possibleCandidates = getCandidates(answer, answers).filter((value) => {
  //   return a <= value.length && value.length <= b;
  // });
  // console.log(possibleCandidates);
  // if (possibleCandidates.length > 0) return [a, b];
  // else {
  //   console.log(a, b);
  //   return [false, false];
  // }
