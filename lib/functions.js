...
// get random range for answers in speed mode
export const getRandomRange = (answer, answers, alpha) => {
  ...
  // 出力する範囲に存在する駅数の最小値をalphaによって決める
  const minimumThreshold = Math.floor(candidates.length ** alpha);

  // 各駅名の長さごとのカウントを保持するオブジェクト
  const counts = {};

  // 各駅に対して、その長さのカウントを1増やす
  ...
  // 最小値と最大値の間で、結果として出力する範囲の中心をランダムに定める
  ...
  // 最小値と最大値の間で、結果として出力する閾値以上の駅数を含む文字数の範囲を決定する
  while (nextCandidatesCount < minimumThreshold) {
    index += (-1) ** i * (i + 1); // center左右交互にに探索していく

    
    if (counts[index]) nextCandidatesCount += counts[index];

    if (i % 2 === 0) { // 偶数回目の探索は大きい側
